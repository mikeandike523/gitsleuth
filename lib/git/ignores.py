...
def get_ignore_checker_given_repo_root_and_containing_directory(repo_root_directory: str, directory: Optional[str] = None) -> Callable[[str], bool]:
    """
    ...
    if not is_child_of(repo_root_directory, directory):
        raise Exception("The subdirectory from which to establish a robust checker was not in the repo")


    def combinator_boolean_or(callables: List[Callable[[str], bool]]) -> Callable[[str], bool]:
    ...
    return combinator_boolean_or(
        list(map(get_is_ignored_checker, directory_list)))
...
def get_robust_ignore_checker(repo_root_directory: str) -> Callable[[str], bool]:
    ...
    def make_checker(containing_directory_path: str) -> Callable[[str], bool]:
        ...
        checker = get_ignore_checker_given_repo_root_and_containing_directory(repo_root_directory, containing_directory_path)
...
def collect_included_or_excluded_files(repo_root_directory: str, collect_mode: CollectMode, path_return_mode: Optional[PathReturnMode] = PathReturnMode.RELATIVE) -> List[str]:
    ...
    def should_collect(absolute_path: str) -> bool:
        return (not os.path.isdir(absolute_path)) and (checker(absolute_path) if collect_mode == CollectMode.EXCLUDED else not checker(absolute_path))
    collected_relpaths = []
...
class RepoIgnoreChecker:
    ...
    def collect_included_files(self, return_mode: Optional[PathReturnMode] = PathReturnMode.RELATIVE) -> List[str]:
        return collect_included_or_excluded_files(self.repo_root_directory, CollectMode.INCLUDED, return_mode)
    
    def collect_excluded_files(self, return_mode: Optional[PathReturnMode] = PathReturnMode.RELATIVE) -> List[str]:
        return collect_included_or_excluded_files(self.repo_root_directory, CollectMode.EXCLUDED, return_mode)
